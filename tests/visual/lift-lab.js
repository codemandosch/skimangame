...
const layout=getLiftLayout(),scene=new THREE.Scene(),camera=new THREE.PerspectiveCamera(55,innerWidth/innerHeight,.5,30000);
...
function reset(mode='drop') {
  ...
  auto=mode==='clear';cleared=false;pop=false;turn=0;accumulator=0;keys.clear();effects.reset();world.update(state,camera);
...
function frame(now) {
  ...
  world.update(state,camera);rider.update(state,dt);effects.update(state,dt);
