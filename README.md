...
The **East Face Park Line** is built into Blackridge: a 160 m wide groomed strip follows the face's average downhill pitch, with feathered edges joining the surrounding mountain. The drop-in and park features sit 100 m uphill from their original locations. Four large curved kickers (East Face Air, Long Shot, Big Sky and Home Stretch) grow from 32 / 40 / 44 / 48 m tall with long, gentle approaches, separated by runouts of about 88 m. Their separate landings are 140 / 144 / 146 / 148 m wide and 96 / 102 / 105 / 108 m long: the front half has been removed, and the landing starts rise 24 / 28 / 30 / 32 m above the groomed strip for steeper descents. Each landing starts with a plateau covering 35% of its length and following the groomed mountain pitch before descending more steeply to the runout. Gaps are 95 / 92.65 / 86 / 79.3 m, short enough that the capped top speed still carries a tucked rider onto every deck. A lower spine jump finishes the line with a curved approach, roughly 25 m of rise above the groomed strip, and a straight 16 m horizontal takeoff edge. Its approximately 65° lip redirects approach momentum forward and upward without the former takeoff speed loss. Left/right controls let you choose the landing faces on either side of its 4 m wide, 112 m long central plateau, followed by a 120 m taper. Surface travel and airborne motion carry the same approach momentum through the curved ramp, and charged pops work consistently before and just after the lip. A height pole midway along the spine measures 0–60 m above the lip, with metre ticks and labels every 10 m. Blue snow dye marks the lips and landing zones, and orange flags mark the lane edges. Use **Ride East Face Park Line** from the start or pause screen to reach the line; Backspace restarts at that drop-in, and **Back to the summit** returns to the summit.

Blackridge has approximately 1,900 vertical metres of relief. Descend any side, traverse between six named areas, and finish after landing in the outer base runout. Staggered wind lips, cliff shelves, connected ridges and catch bowls replace the former corridor. Natural lips have rounded crests, so the two-metre terrain triangles never chew them into teeth, and each crest line bows and wanders slightly differently (straight along its centre line, where approaches aim). Takeoff physics measures from the same curved crest. Lift and park kickers stay straight. The small round summit platform blends into steep slopes in every direction, with twelve early cornices and further side hits connecting the major jumps. Another 100 small wind lips fill the gaps: 20 on the upper mountain, 35 in the middle and 45 lower down, with space reserved for existing approaches and landings. A further 359 broad snowfield rollers connect the gaps, while deep basin exits retain smooth central paths. A terrain coverage regression checks that every sampled point on the main mountain has a takeoff within 200 m. Distant terrain retains more detail so upcoming lips remain visible. All sloped mountain terrain is snow-covered; steepness alone never exposes rock. Signature jumps produce long flights, with exceptional fast, charged approaches exceeding twelve seconds in simulation. Takeoffs respond to the terrain and approach direction. A contour minimap and altitude display help with orientation; there are no mandatory gates or ordered jumps. Terrain rendering near the skier shares its two-metre triangles with collision. Rider alignment accounts for both forward and sideways slope, and the camera checks terrain clearance throughout turns.
...
Blackridge stands in a forested resort valley ringed by eroded, glaciated ranges that climb to about 3,500 m, with further ridges fading into haze out to 30 km. A single fixed afternoon sun (62° azimuth, 34° elevation) drives everything: the sky, the real-time light, baked terrain shadows and the aerial haze.

- **Light and atmosphere.** A physically inspired sky with a warm aureole, cirrus and altocumulus lights the scene through a pre-filtered environment map, so shaded snow turns sky-blue and undersides pick up snow bounce. Every material shares one aerial-perspective model: haze thickens toward the valley floor, turns distant ridges blue and glows around the sun. Frames render to an HDR target with MSAA, then get a restrained bloom, a filmic grade (cool shadows, warm highlights, vibrance, vignette, dither) and a subtle lens flare that ridges can hide.
- **Snow.** Blackridge's snow is shading only; every triangle stays skiable snow and the collision surface is unchanged. All terrain shadows come from baked sun and sky visibility (ridge shadows and soft fir shadows), so they never shift with the rider. The real-time shadow map adds only the rider, trees, logs and lift on top. Light bounced from sunlit slopes keeps the lee faces bright. Steep faces show subtle flutes that follow the fall line. Open slopes carry sastrugi, ripples, powder lumps and wind-crust sheen, and sunlit crystals sparkle. Snow scatters a cool glow around the terminator, and from a distance ridge and gully relief is gently exaggerated so the mountain's form stays readable.
- **Landscape.** The ranges come from warped ridged noise, 900k-droplet hydraulic erosion and talus relaxation. Their shader places bare rock on steep faces, snow on ledges and in couloirs, blue glacier ice in high bowls, and painted conifer canopy below the tree line. Around 50,000 valley trees are placed away from roads, buildings, the lift plaza and the ski faces; they draw as low-poly spruces near the camera and as lit cards beyond 600 m, with a dithered crossfade between the two.
- **Sky life.** Hundreds of billowing cards form a sea of cloud in the distant valleys, banner clouds trail from the highest peaks, spindrift streams off the summit, wood smoke drifts from village chimneys, and diamond dust glints in the air around the camera.
- **On the snow.** Powder spray is lit and glows when backlit. Ski tracks and landing craters are lit surface impressions that follow the collision heightfield and never deform it; restarting clears them.
...
- `src/world.js`: assembles the sun, sky, haze, mountain, landscape, clouds, particles and chairlift.
- `src/sun.js`, `src/atmosphere.js`, `src/alpine-sky.js`: the shared sun, aerial-perspective haze (installed into every material's fog chunk), sky dome and sky environment lighting.
- `src/render-pipeline.js`: HDR composer, bloom, filmic grade, vignette, dither and lens flare.
- `src/snow-surface.js`: Blackridge snow shading (baked light, flutes, sastrugi, sparkle, terminator scattering).
- `src/landscape.js`, `src/landscape-layout.js`, `src/png-data.js`: the baked valley and ranges, valley forests with card LODs and the distant horizon ring.
- `src/clouds.js`, `src/ambient-particles.js`: cloud sea, banner clouds, summit spindrift, chimney smoke and diamond dust.
- `scripts/bake-landscape.mjs`: offline erosion and lighting bake for `public/terrain/`.
